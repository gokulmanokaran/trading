...
function App() {
  ...
  useEffect(() => { if (page !== 'calls') return; const load = async () => { try { const response = await fetch(apiUrl(`/api/calls?market=${market}`)); setCalls(await response.json()) } catch { setCalls([]) } }; load(); const timer = setInterval(load, 10000); return () => clearInterval(timer) }, [page, market])
