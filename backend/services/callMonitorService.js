...
export function createCallMonitor({ supabase, token }) {
  return async function monitor() {
    ...
    const { data: calls, error } = await supabase.from('trading_calls').select('*').in('status', ['ACTIVE', 'TARGET_1_HIT'])
    ...
    for (const call of calls || []) {
      ...
      else if (call.status === 'ACTIVE' && marketPrice >= call.target_1) { status = 'TARGET_1_HIT'; patch.target_1_at = new Date().toISOString() }
